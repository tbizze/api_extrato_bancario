            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="flex justify-between items-center">
                        <h1 class="py-5 text-xl">Editar Usuário</h1>
                    </div>

                    <form action="{{ route('users.update', $user) }}" method="POST" class="w-full max-w-lg">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="flex flex-wrap -mx-3 mb-6">
                            <div class="w-full md:w-2/2 px-3 mb-6 md:mb-0">
                                ...
                                <input
                                    ...
                                    value="{{ old('email', $user->email) }}" placeholder="test@test.com">
                                @error('email')
                                    <div class="text-xs text-red-600">{{ $message }}</div>
                                @enderror
                            </div>
                                    ...
                                    <select
                                        ...
                                        @foreach ($companies as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $item->id == $user->company_id ? 'selected' : '' }}>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                    <div
                                ...
                                <div class="relative">
                                    <select
                                        ...
                                        @foreach ($tipos as $item)
                                            <option value="{{ $item['id'] }}"
                                                {{ $item['id'] == $user->is_superuser ? 'selected' : '' }}>
                                                {{ $item['name'] }}</option>
                                        @endforeach
                                    </select>
                                    <div
